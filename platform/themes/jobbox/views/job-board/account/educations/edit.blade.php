    ...
    <form action="{{ route('public.account.educations.edit.update', $education->id) }}" method="post">
        <div class="row">
            <div class="col-12">
                ...
                <div class="form-group">
                    <label class="font-sm color-text-mutted mb-10" for="specialized">{{ __('School Level') }}</label>
                    <input type="text" class="form-control @error('position') is-invalid @enderror" id="position"
                           name="specialized" value="{{ old('specialized', $education) }}" placeholder="{{ __('Enter School Level') }}"/>
                    @error('specialized')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
